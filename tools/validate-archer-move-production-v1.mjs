...
// Read-only validator for the Archer chibi Move v1 production candidate.
...
const DIR = 'assets/units/hero.archer/move-chibi-v1';
const SIDECAR = 'assets/units/hero.archer/hero.archer_move_chibi_v1.json';
const SOURCE_MAP = `${DIR}/source-map.json`;
const METRICS = 'docs/reviews/archer-move-production-v1-validation.json';
const CONTRACT = 'data/design/archer-move-production-v1.json';
const N = 8, W = 640, H = 960, IDENTITY_Y = 610;
const errors = [];
...
function footRegion(frame, x0, x1) {
  let maxY = -1, contact = 0;
  for (let y = 700; y < 900; y++) for (let x = x0; x < x1; x++) {
    const alpha = frame.pixels[(y * W + x) * 4 + 3];
    if (alpha > 8) { maxY = Math.max(maxY, y); if (y >= 852 && y <= 854) contact++; }
  }
  return { maxY, contact };
}
...
for (let index = 0; index < N; index++) {
  const path = `${DIR}/hero.archer_move_${String(index).padStart(3, '0')}.png`;
  ...
  try {
    ...
    assert(alpha.minX >= 60 && alpha.maxX <= 580 && alpha.minY >= 110 && alpha.maxY <= 860, `frame ${index}: alpha bounds imply crop or scale drift`);
    frames.push({ index, path, digest, frame, alpha });
  } catch (error) { errors.push(`frame ${index}: unreadable PNG (${error.message})`); }
}
assert(!fs.existsSync(`${DIR}/hero.archer_move_008.png`), 'unexpected frame 008');
assert(frames.length === N, `decoded frame count ${frames.length}, expected ${N}`);

if (frames.length === N && source?.pixels) {
  const identityBytes = IDENTITY_Y * W * 4;
  for (const { index, frame } of frames) {
    assert(frame.pixels.subarray(0, identityBytes).equals(source.pixels.subarray(0, identityBytes)), `frame ${index}: upper identity zone y<${IDENTITY_Y} differs from Neutral Master`);
    assert(premultipliedMae(source, frame) < 0.015, `frame ${index}: source delta exceeds identity threshold 0.015`);
  }
  const adjacent = frames.map(({ frame }, index) => premultipliedMae(frame, frames[(index + 1) % N].frame));
  const seam = adjacent[N - 1], maximum = Math.max(...adjacent);
  assert(seam <= maximum + 1e-12, `loop seam ${seam} exceeds maximum adjacent delta ${maximum}`);
  assert(maximum < 0.011, `maximum adjacent visual delta ${maximum} exceeds Move threshold 0.011`);

  const leftMarkerCharacterLeft = footRegion(frames[2].frame, 315, 450);
  const leftMarkerOpposite = footRegion(frames[2].frame, 120, 315);
  assert(leftMarkerCharacterLeft.maxY === 854 && leftMarkerCharacterLeft.contact > 0, 'frame 2: character-left foot must contact y=854');
  assert(leftMarkerOpposite.maxY <= 840 && leftMarkerOpposite.contact === 0, 'frame 2: opposite foot must be visibly lifted');
  const rightMarkerCharacterRight = footRegion(frames[6].frame, 120, 315);
  const rightMarkerOpposite = footRegion(frames[6].frame, 315, 450);
  assert(rightMarkerCharacterRight.maxY === 854 && rightMarkerCharacterRight.contact > 0, 'frame 6: character-right foot must contact y=854');
  assert(rightMarkerOpposite.maxY <= 848 && rightMarkerOpposite.contact === 0, 'frame 6: opposite foot must be visibly lifted');
}

if (sidecar) {
  assert(sidecar.assetId === 'hero.archer.move.chibi-production-candidate.v1', 'sidecar assetId mismatch');
  assert(sidecar.unitId === 'hero.archer' && sidecar.state === 'move', 'sidecar unit/state mismatch');
  assert(sidecar.fps === 12 && sidecar.frameCount === 8 && sidecar.loop === true, 'sidecar timing mismatch');
  ...
  assert(JSON.stringify(sidecar.eventMarkers) === '[{"name":"leftFootstepCue","normalizedTime":0.25},{"name":"rightFootstepCue","normalizedTime":0.75}]', 'sidecar marker contract mismatch');
  assert(sidecar.neutralMasterApproved === true && sidecar.idlePackageApproved === true, 'sidecar source approval flags mismatch');
  for (const key of ['movePackageApproved', 'canonicalApproved', 'runtimeEligible', 'runtimeIntegrated']) assert(sidecar[key] === false, `sidecar ${key} must be false`);
}
...
if (sourceMap) {
  ...
  assert(sourceMap.productionMethod?.upperIdentityLockExclusiveY === IDENTITY_Y, 'source-map identity lock mismatch');
  ...
  for (const key of ['movePackageApproved', 'canonicalApproved', 'runtimeEligible', 'runtimeIntegrated']) assert(sourceMap[key] === false, `source-map ${key} must be false`);
}

if (metrics) {
  assert(metrics.loopContinuity?.seamNotGreaterThanMaximumAdjacent === true, 'metrics loop seam invariant failed');
  assert(metrics.rootMotion?.horizontalCentroidRangePixels < 2, 'horizontal centroid range must remain below 2 px');
  assert(metrics.rootMotion?.verticalCentroidRangePixels < 4, 'vertical centroid range must remain below 4 px');
  ...
  assert(JSON.stringify(metrics.anchorDecision?.supportFootBaselineRangeY) === '[852,854]', 'anchor support baseline range mismatch');
  ...
  assert(metrics.status?.neutralMasterApproved === true && metrics.status?.idlePackageApproved === true, 'metrics source approval flags mismatch');
  for (const key of ['movePackageApproved', 'canonicalApproved', 'runtimeEligible', 'runtimeIntegrated', 'attackGenerated']) assert(metrics.status?.[key] === false, `metrics ${key} must be false`);
}

if (contract) {
  assert(contract.characterId === 'hero.archer' && contract.state === 'move', 'contract character/state mismatch');
  assert(contract.sourceNeutralMaster?.sha256 === SOURCE_SHA, 'contract Neutral Master SHA mismatch');
  assert(contract.neutralMasterApproved === true && contract.idlePackageApproved === true, 'contract source approval flags mismatch');
  for (const key of ['movePackageApproved', 'canonicalApproved', 'runtimeEligible', 'runtimeIntegrated', 'attackGenerated']) assert(contract[key] === false, `contract ${key} must be false`);
...
if (errors.length) {
  console.error('Archer Move Production v1 validation failed:');
  for (const error of errors) console.error(`- ${error}`);
  process.exit(1);
}
console.log('Archer Move Production v1 validation passed: exact approved sources, 8 unique RGBA frames, locked upper identity, in-place gait, marker contacts, anchor/bounds, loop, provenance, and approval invariants verified.');
