...
const DIR = 'assets/units/monster.slime/idle';
const SIDECAR = 'assets/units/monster.slime/monster.slime_idle_motiontest.json';
const SOURCE_MAP = `${DIR}/source-map.json`;
const CONTRACT = 'data/design/pilot-idle-motion-production-contract-v1.json';
const CONTACT = 'docs/assets/review/slime-idle-motion-test-v1-contact-sheet.png';
const GIF = 'docs/assets/review/slime-idle-motion-test-v1-preview.gif';
const N = 8, W = 512, H = 512;
const errors = [];
...
function metrics(pixels) {
  ...
  let largestComponent = 0, componentCount = 0;
  ...
  return {
    ...
    borderOpaque,
    componentCount,
    detachedPixels: count - largestComponent
  };
}

const expectedNames = Array.from({ length: N }, (_, index) => `monster.slime_idle_${String(index).padStart(3, '0')}.png`);
...
assert(!fs.existsSync(`${DIR}/monster.slime_idle_008.png`), 'unexpected frame 008');
...
for (const name of expectedNames) {
  ...
  try {
    ...
    assert(png.ihdr.w === W && png.ihdr.h === H, `${name}: canvas must be 512x512`);
    ...
    if (png.pixels) {
      ...
      assert(measure.opaqueFraction > 0.25 && measure.opaqueFraction < 0.75, `${name}: opaque fraction ${measure.opaqueFraction.toFixed(4)} implausible`);
      assert(measure.detachedPixels <= 8, `${name}: detached alpha residue ${measure.detachedPixels}px (possible checkerboard/residue)`);
...
if (frames.length === N) {
  ...
  assert(centroidSpread <= 0.75, `horizontal centroid spread ${centroidSpread.toFixed(3)}px exceeds 0.75px`);
  const baselines = frames.map((frame) => frame.baselineY);
  assert(new Set(baselines).size === 1 && baselines[0] === 460, `foot/root baseline must stay y=460 (got ${baselines.join(',')})`);
  assert(Math.abs(baselines[0] - (H * 0.9 - 0.8)) <= 1, 'baseline does not align with anchor [0.5,0.9]');
  ...
  assert(seamMean < 0.03, `007->000 normalized mean seam ${seamMean.toFixed(5)} exceeds 0.03`);
  ...
  console.log(`Forensics: detached alpha residue=${frames.map((frame) => frame.detachedPixels).join(',')}; opaque border=0 for 8/8.`);
}
...
if (fs.existsSync(SIDECAR)) {
  try {
    ...
    assert(sidecar.unitId === 'monster.slime' && sidecar.state === 'idle', 'sidecar identity/state mismatch');
    assert(sidecar.fps === 8 && sidecar.frameCount === 8 && sidecar.loop === true, 'sidecar timing mismatch');
    assert(JSON.stringify(sidecar.canvas) === JSON.stringify([512, 512]), 'sidecar canvas mismatch');
    assert(JSON.stringify(sidecar.anchor) === JSON.stringify([0.5, 0.9]), 'sidecar anchor mismatch');
...
if (fs.existsSync(SOURCE_MAP)) {
  try {
    ...
    assert(sourceMap.identityMaster?.sha256 === '12ca260a001dbda6177f29565e9df2c369c09216c889974aeee8b12fb5ae04f9', 'identity master hash mismatch');
...
if (fs.existsSync(CONTRACT)) {
  try {
    ...
    const locked = contract.sourceIntegrity?.['monster.slime/move'] ?? [];
    for (let index = 0; index < 8; index++) {
      const path = `assets/units/monster.slime/move/monster.slime_move_${String(index).padStart(3, '0')}.png`;
      assert(sha256(path) === locked[index], `locked Slime Move source changed: frame ${index}`);
    }
    const sourceSidecar = 'assets/units/monster.slime/monster.slime_move_motiontest.json';
    assert(sha256(sourceSidecar) === contract.sourceIntegrity?.sidecars?.[sourceSidecar], 'locked Slime Move sidecar changed');
...
if (errors.length) {
  console.error('Slime idle frames validation failed:');
  for (const error of errors) console.error(`- ${error}`);
  process.exit(1);
}

console.log('Slime idle frames validation passed (8 unique real RGBA frames, stable baseline/centroid, true alpha, source locks and sidecar consistent).');
