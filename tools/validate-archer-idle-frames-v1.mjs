...
// Validates the REAL Archer idle motion-test binaries (PR #39 source, integrated on the pilot
...
// sidecar consistency (fps 8, frameCount 8, loop true, anchor [0.5,0.92], NO event markers,
// rootMotion in-place, canonicalApproved false). Never modifies any file.
...
const DIR = 'assets/units/hero.archer/idle';
const SIDECAR = 'assets/units/hero.archer/hero.archer_idle_motiontest.json';
...
for (let i = 0; i < N; i++) {
  const name = `hero.archer_idle_${String(i).padStart(3, '0')}.png`;
  const path = `${DIR}/${name}`;
...
assert(!fs.existsSync(`${DIR}/hero.archer_idle_008.png`), 'unexpected extra frame 008 present');
...
if (fs.existsSync(SIDECAR)) {
  ...
  if (meta) {
    ...
    assert(meta.state === 'idle', 'sidecar state must be idle');
    assert(meta.fps === 8, 'sidecar fps must be 8');
    ...
    assert(Array.isArray(meta.eventMarkers) && meta.eventMarkers.length === 0, 'idle must declare NO event markers');
...
if (errors.length) {
  console.error('Archer idle frames validation failed:');
  for (const e of errors) console.error('- ' + e);
  process.exit(1);
}
console.log('Archer idle frames validation passed (8 real frames, unique hashes, structure/alpha/border/sidecar consistent).');
