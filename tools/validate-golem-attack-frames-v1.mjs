...
// Validates the REAL Golem attack motion-test binaries for THIS motion-test round:
// naming 000-007, exactly 8 frames, structurally valid PNGs (signature, IHDR, IDAT inflates to
// the exact expected length, unfilter 0-4), 8-bit RGBA, identical 640x640 canvas, alpha genuinely
// used (transparent corners, no opaque pixels on the canvas border, plausible opaque fraction),
// all frame hashes unique (no accidental duplicates), and sidecar consistency (fps 12,
// frameCount 8, loop false, anchor [0.5,0.94], impactCue @ 0.75, rootMotion in-place,
// canonicalApproved false, canvas matching the actual files).
// NOTE: this round's 8-frame / impactCue@0.75 values deliberately deviate from the Golem
// Production Plan v1 (12 frames, impactCue@0.58) — a documented motion-test experiment, not a
// silent plan change. Never modifies any file.
...
const DIR = 'assets/units/monster.golem/attack';
const SIDECAR = 'assets/units/monster.golem/monster.golem_attack_motiontest.json';
...
for (let i = 0; i < N; i++) {
  const name = `monster.golem_attack_${String(i).padStart(3, '0')}.png`;
  const path = `${DIR}/${name}`;
...
assert(!fs.existsSync(`${DIR}/monster.golem_attack_008.png`), 'unexpected extra frame 008 present');
...
if (fs.existsSync(SIDECAR)) {
  ...
  if (meta) {
    assert(meta.unitId === 'monster.golem', 'sidecar unitId must be monster.golem');
    ...
    assert(JSON.stringify(meta.anchor) === JSON.stringify([0.5, 0.94]), 'sidecar anchor must be [0.5, 0.94]');
    ...
    assert(Array.isArray(meta.eventMarkers) && meta.eventMarkers.length === 1 && meta.eventMarkers[0].name === 'impactCue' && meta.eventMarkers[0].normalizedTime === 0.75, 'sidecar must declare exactly impactCue @ 0.75 (this motion-test round)');
...
if (errors.length) {
  console.error('Golem attack frames validation failed:');
  for (const e of errors) console.error('- ' + e);
  process.exit(1);
}
console.log('Golem attack frames validation passed (8 real frames, unique hashes, structure/alpha/border/sidecar consistent).');
