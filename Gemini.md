...
**Arena Layout Pass: โครง UX/UI แนวนอนตามภาพอ้างอิง (เสร็จแล้ว)**
- รอบนี้เป็น layout/presentation ล้วน — **ไม่มี gameplay logic เปลี่ยนแม้แต่จุดเดียว** (combat/shop/
  merge/star/link/item/wave เดิมทั้งหมด ยืนยันด้วย regression ครบ)
- **Top Status Bar เตี้ยลง** (จบที่ ~41px): HP ผู้เล่นแสดงเป็น **หลอด 100/100** — แปลงจากโควตาแพ้เดิม
  `(MAX_LOSSES-losses)/MAX_LOSSES*100` เป็นการแสดงผลล้วนๆ (ลอจิกแพ้/จบเกมยังอิง losses เดิม) + Wave
  x/15 + Gold + **Lv./EXP + ปุ่ม +EXP ย้ายจากมุมล่างซ้ายขึ้นแถบบน** (id เดิมทุกตัว ลอจิกไม่แตะ) + Speed
  — มุมล่างซ้ายที่ว่างลงยกให้ปุ่มกระเป๋าไอเทม (ย้ายจากขวาที่ตอนนี้ชนคอลัมน์ขวาใหม่ — แผงกางออกจากซ้าย
  `translateX(-120%)` แทน)
- **Monster Panel ใหม่ (ขวาบน)**: ศัตรูของเวฟปัจจุบัน จัดกลุ่มตามชนิด (sprite) — icon จาก
  `ASSET_META[sprite].path` (มอนไม่มีภาพ เช่น Slime/OrcBrute ใช้ swatch สีจาก placeholderColor ตาม
  asset policy — ไม่เจน asset ใหม่), ชื่อ (ตัดเลข/อักษรท้าย "Slime 1"→"Slime"), ตัวนับรอด x/N, หลอด
  HP รวมของชนิด, บอสมีกรอบทอง+👑 — ช่วง shop เป็น **พรีวิวจาก `buildWave(wave)`** (อ่านอย่างเดียว
  ไม่ spawn ไม่ mutate), ช่วง battle อ่านยูนิตศัตรูจริง — **rebuild DOM เฉพาะตอน renderUI**, ระหว่าง
  ต่อสู้ `updateMonsterPanelBars()` (เรียกคู่กับ updateTeamHpBars ใน animate) แก้แค่ width/ตัวเลขของ
  แถวเดิม — แผงทีมในสนาม (เดิม) ย้ายไปซ้อนใต้ใน `#rightCol` (flex column ชิดขวา)
- **Tile highlight เฉพาะตอนวางตัว**: สีโซนแบบเดิม (เขียวจัด/น้ำตาลเข้ม) เปลี่ยนเป็นโทนจางกลืนพื้นเวที
  (`baseTint` เก็บใน userData) — `updateTileHighlights()` ทำงานเมื่อ `selectedUnit` มีค่า หรือกำลังลาก
  (`unitDrag.moved`): ช่องผู้เล่นว่าง = เขียวอ่อน (แถวม้านั่ง = ฟ้าอ่อน แยกโซน), ช่องใช้งานอยู่ = แดง
  โปร่งใส (opacity 0.72), แถวศัตรูไม่แตะ — เรียกจาก renderUI + จุดเริ่ม/จบ/ยกเลิกลาก (จุดจบลากต้อง
  เรียกตรงๆ เพราะลากที่ปล่อยลงช่องไม่ valid ไม่ผ่าน renderUI) — แตะแค่ material.color/opacity
  **ไม่แตะพิกัด/raycast/กติกาวางตัวเลย** (raycast ยังใช้ getBoundingClientRect ของ canvas เดิม ✓)
- **กรอบเวที (stage frame)**: ขอบหินโทนน้ำเงินเทา (BoxGeometry สูง 0.1) รอบกระดาน 8x7 + เสาหัวมุม
  ยอดทอง — geometry ล้วนตาม asset policy, อยู่นอกพื้นที่ไทล์และไม่อยู่ในลิสต์ raycast ใดๆ จึงไม่กระทบ
  การคลิก และไม่บังตัวละคร/หลอดเลือด — พาเลตต์ CSS ขยับเป็นน้ำเงินเข้ม/เทา/ทอง (--panel-bg #151a26
  ฯลฯ) ตาม art direction
- **Responsive/safe-area**: `viewport-fit=cover` + `env(safe-area-inset-*)` ที่ topbar/แผงซ้ายขวา/มุม
  ล่าง, จอเตี้ย ≤430px ซ่อน phaseLabel (ปุ่มเริ่มต่อสู้บอกเฟสอยู่แล้ว ข้อมูลสำคัญอื่นครบ) + ย่อแผง,
  จอ ≥900px ขยายแผง/หลอด/ไอคอน — ไม่มี horizontal scroll (ยืนยันใน test)
- Acceptance ผ่านครบ (Playwright): เปิดเกมไม่มีจอดำ/ไม่มี error, top bar ครบทุกช่อง (HP 100/100 จาง
  ลงจริงเมื่อแพ้), Monster Panel พรีวิว Slime 3/3 → สด 1/3 หลอด 4% ระหว่างต่อสู้ **บนแถว DOM เดิม**,
  highlight เขียว/แดงเปิด-ปิดถูกจังหวะ แถวศัตรูไม่เปลี่ยน, ซื้อ/วาง/ลาก/ขาย/เปิดปิดร้าน/ซื้อ EXP จาก
  แถบบน/กระเป๋าฝั่งซ้ายทำงานครบ, Link 3 ตัว + บัฟถูกต้อง, evolution/star-combine เหมือนเดิม, เล่นจริง
  3 เวฟ x4 ไม่มี NaN — screenshot ครบ 3 ขนาดจอ (desktop 1280x800 / มือถือแนวนอน 844x390 / iPad
  1194x834)

**Arena Visual Upgrade: พื้น/กรอบเวทีจาก placeholder เดิมให้ดูเป็น Arena สมบูรณ์ขึ้น (เสร็จแล้ว)**
- รอบนี้เป็น visual/decoration ล้วน — **ไม่มี gameplay logic เปลี่ยนแม้แต่จุดเดียว** (`GRID_COLS`/
  `GRID_ROWS`/`gridToWorld`/raycast/placement/combat/Link/Monster Panel/Shop/Star/Wave/Economy เดิม
  ทั้งหมด ยืนยันด้วย regression: pickTileAtScreenPoint ยังชี้ช่องถูกต้อง, Link 3 ตัว+บัฟทำงาน,
  Monster Panel/Star fn ยังอยู่ครบ, เล่นจริง 1 เวฟ x4 ไม่มี NaN)
- **ไทล์กระดาน**: เปลี่ยน material จาก `MeshBasicMaterial` (ไม่รับแสง) เป็น `MeshStandardMaterial`
  (roughness:1, metalness:0) — ให้พื้นกระดานรับแสงจาก DirectionalLight จริง ดูเป็นพื้นหินของเวทีแทน
  กริด UI แบนๆ โดย `updateTileHighlights()`/`baseTint`/สี เขียว-ฟ้า-แดงตอนเลือก/ลากยังทำงานเหมือนเดิม
  ทุกจุด (แค่ตั้งค่า `.material.color`/`.opacity` เหมือนเดิม)
- **พื้นเวทีรอบนอก (arena floor)**: แทนที่พื้นสีล้วน `0x1a1c26` ด้วยลายหินโมเสก procedural วาดด้วย
  canvas (`arenaFloorTexture()`, RepeatWrapping ปูซ้ำ) + `MeshStandardMaterial` — ไม่มี asset ภาพ
  ใหม่เลยตาม policy
- **กรอบเวที**: เสามุมเดิม (เตี้ย, แค่หัวเสาทอง) ปรับให้สูงขึ้นเป็นเสาเวทีจริง (0.9 หน่วย) — เพิ่ม
  **ธงเวที** (`clothBannerTexture()`, ผ้าสองโทน+สัญลักษณ์เพชรเรขาคณิตล้วน ไม่มีโลโก้/ตัวละครจากภาพ
  อ้างอิง) ทุกมุม + **คบเพลิง** (เสาไม้+เปลวไฟ MeshBasicMaterial+ประกาย Sprite แบบ AdditiveBlending)
  เฉพาะ 2 มุมฝั่งม้านั่ง (ใกล้กล้อง/ผู้เล่นมากสุด) — **ไม่เพิ่ม PointLight จริง** ตาม art direction
  (ห้ามไฟเยอะ/เอฟเฟกต์หนัก) ใช้ sprite เรืองแสงแทนเพื่อคุม cost การเรนเดอร์ให้ใกล้เดิม
  - เสา/ธง/คบเพลิง/ขอบหิน/เศษหิน/เสาหักเดิมทั้งหมดอยู่นอกขอบ `halfW`/`halfD` ของกระดาน และไม่เคยถูก
    เพิ่มเข้า `tileMeshes` หรือ raycast list ใดๆ — ไม่กระทบการคลิก/วางตัว ไม่บังตัวละคร/หลอด HP/มานา/
    Link ring/ป้ายโซ่ (ตำแหน่งสูงสุดของคบเพลิง/ธงอยู่นอกพื้นที่ตัวละครเดินสัญจรทั้งหมด)
- **Reference image**: ผู้ใช้แนบภาพ mock-up Auto-Battler แบบ mobile (กระดานหินสี่เหลี่ยม, เสามุม+ธง+
  คบเพลิง, บรรยากาศป่า) ใช้เป็น **art direction เท่านั้น** — ไม่ได้ copy ตัวละคร/โลโก้/asset เฉพาะจาก
  ภาพเลย ทุกอย่างสร้างจาก geometry + canvas texture ที่วาดขึ้นเองทั้งหมด
- Asset policy: ตรวจ `assets/` ก่อนแล้วพบว่าไม่มีไฟล์ arena/banner/torch สำเร็จรูป และไฟล์เดียวที่ใกล้
  เคียง (`assets/v5/board_ground.jpg`) มีสถานะลิขสิทธิ์ "unknown" ในเอกสารเดิม จึงเลือกไม่ใช้และสร้าง
  ใหม่ด้วย canvas/geometry ล้วนแทน (ไม่มีไฟล์ asset ใหม่เพิ่มเข้า repo เลยรอบนี้ — เป็นโค้ด runtime
  ทั้งหมด เหมือน `stoneTileTexture()` เดิมที่ไม่เคยอยู่ใน manifest เพราะไม่ใช่ไฟล์)
- ทดสอบผ่าน Playwright ครบ: ไม่มีจอดำ/console error (นอกจาก favicon 404 ที่ไม่เกี่ยวกับเกม), ซื้อ/วาง/
  ลากฮีโร่ตรงช่อง (raycast ยืนยันตรง), highlight เขียว/แดงถูกสี, Link 3 ตัวทำงาน, Monster Panel/Star
  fn ยังอยู่, เล่นจริง 1 เวฟ x4 ไม่มี NaN, ทดสอบเต็มจอ 3 สัดส่วน (กว้าง 1600x700, เกือบสี่เหลี่ยม
  900x860, จอเตี้ย 900x380) ไม่มี horizontal scroll ทุกขนาด กระดานไม่ถูกบีบ

**Camera/Arena Scale Pass: ขยาย Arena ~15.5% ตาม Reference (เสร็จแล้ว)**
- รอบนี้แก้เฉพาะ `layoutBoard()`/`BOARD_FILL_RATIO` ล้วนๆ — **ไม่แตะ `GRID_COLS`/`GRID_ROWS`/
  `gridToWorld`/`boardCorners`/raycast/placement/Combat/AI/Skill/Item/Wave/Link/Economy/Star/Shop/
  Monster Panel logic ใดๆ เลย** (ยืนยันด้วย regression: raycast วาง/ลากยังตรงช่องเป๊ะ, Link/Monster
  Panel/Star fn ทำงานปกติ, เล่นจริง 1 เวฟ x4 ไม่มี NaN)
- **สิ่งที่ปรับ**: `BOARD_FILL_RATIO` (สัดส่วนความสูงจอที่กระดานกินใน contain-fit frustum เดิม) จาก
  `0.84` → `0.97` — เพราะ `halfW` คำนวณจาก `halfH*aspect` เสมอ (สัดส่วนเดียวกันทุกแกน ไม่บิดภาพ) การ
  ขยับค่านี้ค่าเดียวจึงซูมกล้องเข้าแบบสม่ำเสมอทั้งกว้าง-สูง-ขนาดตัวละครพร้อมกัน — วัด pixel จริงเทียบ
  ก่อน/หลังที่ desktop (1280x800)/มือถือแนวนอน (844x390)/tablet (1194x834) ได้ผลตรงกันทั้ง 3 ขนาดจอ:
  กระดานโต **+15.5%** (0.97/0.84) และตัวละครบนสนามโตตามสัดส่วนเดียวกันพอดี (+15.5% เช่นกัน วัดจาก
  ความสูง sprite บนจอจริง) — อยู่ในช่วง 15-20% ที่ขอ
- **ลองแล้วไม่ใช้**: ทดลองทำ "contain-fit สองแกน" โดยวัดความกว้างแผง Link/Monster จริงจาก DOM
  (`getBoundingClientRect()`) มาคำนวณความกว้างที่เหลือให้กระดานกิน เพื่อหวังใช้พื้นที่ว่างข้างกระดาน
  ที่เห็นในภาพ reference — แต่วัดจริงพบว่าใช้ไม่ได้ผล: ที่ desktop/tablet ความกว้างกลับกลายเป็นเงื่อนไข
  บีบแทนความสูง ทำให้กระดาน **เล็กลงกว่าเดิม** ที่ tablet (ขัดกับ baseline ที่ปุ่ม/แผงลอยทับขอบกระดาน
  อยู่แล้วโดยตั้งใจตาม comment เดิมของ `.floatPanel` CSS) จึงตัดทิ้ง กลับไปใช้ `BOARD_FILL_RATIO` เดี่ยว
  แบบเดิมแต่ปรับค่าขึ้นแทน — ง่ายกว่าและไม่มีผลข้างเคียง
- แผง Link (ซ้าย)/Monster (ขวา) **ไม่ถูกแตะเลยทั้ง CSS width และ logic** — กว้างเท่าเดิมทุกขนาดจอ, Top
  Bar สูงเท่าเดิม (~41px), Bench/Shop ไม่ถูกบีบ (มาร์จิ้นล่างกระดานยังเหลือเยอะพอสำหรับปุ่มร้านค้า/
  เริ่มต่อสู้มุมขวาล่างเหมือนเดิม)
- ทดสอบผ่าน Playwright ครบ 3 ขนาดจอ (desktop/มือถือแนวนอน/tablet): ไม่มีจอดำ/error, กระดานไม่ชนกับ
  topbar/แผงซ้ายขวา/bench, ซื้อ/วาง/**ลากด้วย pointer event จริง**ตรงช่องเป๊ะ (ทดสอบ bench→field),
  Link 3 ตัวทำงาน, Monster Panel/Star fn ยังอยู่ครบ, เล่นจริง 1 เวฟ x4 ไม่มี NaN, ไม่มี horizontal
  scroll ที่ทดสอบเพิ่ม (กว้าง 1600x700/เกือบสี่เหลี่ยม 900x860/จอเตี้ย 900x380)

**Monster Panel Visual Pass: portrait/HP number/Boss section แยกพิเศษ (เสร็จแล้ว)**
- รอบนี้แก้เฉพาะ UI แสดงผลของ Monster Panel — **ไม่แตะ Monster Stats/Wave Schedule/Boss Skill/
  Combat Logic/Camera/Arena Scale/Grid/Link/Shop/Bench/Star/Item/Skill/Economy/AI เลย** ยืนยันด้วย
  regression: preview เวฟ 1 (Slime 3/3) ถูกต้อง, เข้าเวฟบอส (5/15) แสดง boss section ถูกต้อง, เริ่ม
  battle จริงแล้ว HP/จำนวนรอดลดตามจริง (255→231 ระหว่างสู้), แถวเดิม (element เดิม ไม่ถูกสร้างใหม่)
  ระหว่างอัปเดตทุกเฟรม, เล่นจริง 1 เวฟ x4 ไม่มี NaN, raycast/Link/Star ปกติทุกจุด
- **แต่ละแถวมอนสเตอร์ปกติ**: portrait/swatch ใหญ่ขึ้น (30px, 34px ที่จอ ≥900px) ใน `.monPortraitWrap`
  + badge วงกลมมุมล่างขวาโชว์ emoji ตาม `specialBehavior.type` ของมอนสเตอร์ชนิดนั้น (`MONSTER_BEHAVIOR_ICON`:
  hunter🎯/ranged🏹/backline🗡️/frontline🛡️/stun_attack💫 — badge โผล่เฉพาะชนิดที่มีข้อมูลนี้อยู่แล้ว
  จริง ไม่ได้เพิ่ม field ใหม่) + ชื่อชนิด + จำนวนรอด `x/N` (มุมขวาบนของแถว) + หลอด HP สีแดงเข้ม
  (`#a8362f→#d9534f`) พร้อม **เลข current/max ทับกลางหลอด** (ฟิลด์ใหม่ที่ขอเพิ่ม — ใช้ค่า
  `g.hp`/`g.maxHp` เดียวกับที่ขับความกว้างหลอดอยู่แล้ว ไม่ใช่สถิติใหม่ แค่โชว์ตัวเลขเพิ่ม)
- **Boss แยกเป็นส่วนพิเศษจริง**: ย้ายออกจากลิสต์ปกติไปเป็น `#bossSection`/`#bossList` แยกต่างหาก
  (ซ่อนอัตโนมัติเมื่อเวฟไม่มีบอส) มีหัวข้อ "👑 บอสรอบนี้" คั่นด้วยเส้นบนสีทอง, portrait ใหญ่กว่าปกติ
  (40px, 46px ที่จอ ≥900px), กรอบ+ชื่อ+ตัวหลอด HP โทนทอง, **ชื่อสกิลพิเศษ** อ่านจาก
  `BOSS_SKILLS[bossSkillId].name` ที่มีอยู่แล้วในระบบ boss-skill เดิม (Warden's Challenge/Bone
  Breath/Arena Curse) แสดงเป็นตัวเอียงใต้หลอด HP — จอเตี้ย (`max-height:430px`) ซ่อนบรรทัดนี้เพื่อ
  ประหยัดพื้นที่ตามที่ขอ (ชื่อ/จำนวนรอด/HP ไม่ถูกซ่อนเลยในทุกขนาดจอ)
- **สถาปัตยกรรมข้อมูล**: `monsterGroupsForCurrentWave()` เพิ่ม `behaviorType`/`bossSkillId` เข้ากลุ่ม
  (อ่านจาก `m.specialBehavior`/`m.bossSkillId` ที่มีอยู่แล้วในยูนิตจริงและ preview cfg — ยืนยันว่า
  `spawnWave()`→`makeUnit({team:'enemy',...cfg})` spread ฟิลด์เหล่านี้เข้ายูนิตจริงอัตโนมัติอยู่แล้ว
  ไม่ต้องแก้ combat/spawn code เลย) — แยก `buildMonRowEl()` ใช้ร่วมกันทั้งลิสต์ปกติและ boss list ลด
  โค้ดซ้ำ — `updateMonsterPanelBars()` query จาก `#monsterPanel` ทั้งก้อน (ครอบคลุมทั้งสอง list ในทีเดียว)
  อัปเดตแค่ width/เลข HP/ตัวนับ/`.wiped` ของแถวเดิม **ไม่ rebuild DOM ทุกเฟรมเหมือนเดิม**
- **Asset**: ไม่ดาวน์โหลด/เจน asset ใหม่เลยตามนโยบาย — ใช้ portrait เดิมที่มีอยู่ (`ASSET_META`) สำหรับ
  Skeleton/StoneWolf/SpiritArcher/ShadowAssassin/Warden/Golem/BoneDragon/ChampionBig ครบ — **ที่ยังขาด
  จริง**: `Slime` และ `OrcBrute` (ยังไม่มีไฟล์ portrait ในโปรเจกต์ ใช้ swatch สีเขียว/น้ำตาลแทนต่อไปตาม
  เดิม — พบว่ามี `assets/v5/mon_slime.png`/`mon_orc.png` เก่าอยู่ในโปรเจกต์จริง แต่เป็นภาพ AI-generated
  ที่ ASSET_CREDITS.md ระบุสถานะลิขสิทธิ์ "unknown" และไม่เคยถูกใช้ในเกมปัจจุบันมาก่อน จึงไม่หยิบมาใช้
  ต่อ รอ GPT เจนไฟล์ PNG โปร่งใสชุดใหม่ให้แทน)
- ทดสอบผ่าน Playwright ครบ: preview เวฟ 1/เวฟบอส (5, 15) ถูกต้อง, boss section โชว์/ซ่อนถูกจังหวะ,
  portrait บอสใหญ่กว่าจริง (46px vs 34px วัด getBoundingClientRect), เริ่ม battle จริง HP ลดจริง,
  แถว DOM element เดิมไม่ถูกสร้างใหม่ระหว่างอัปเดต, เล่นจริง 1 เวฟ x4 ไม่มี NaN/error, ไม่มี
  horizontal scroll ที่กว้าง 1600x700/เกือบสี่เหลี่ยม 900x860/จอเตี้ย 900x380

**Board Shift-down Fix: แถวบนสุดพ้น topbar จริงทุก breakpoint (เสร็จแล้ว)**
- ผู้ใช้รายงานจากมือถือจริง (screenshot) ว่าหลังขยาย FILL_RATIO เป็น 0.97 แถวบนสุดของกระดานมุดใต้
  topbar เล็กน้อย — แก้ใน `layoutBoard()`: วัดความสูง topbar จริงจาก DOM (`getBoundingClientRect().bottom`
  — สูงไม่เท่ากันแต่ละ breakpoint: 46px desktop / 41px จอเตี้ย) แล้วเลื่อน frustum ลง**เท่าที่ขาดพอดี**
  (+ pad 6px) — จอที่โล่งอยู่แล้วไม่ขยับเลย (deficit ≤ 0) — วัดยืนยัน: boardTop 47-52px > topbarBottom
  ทุกขนาดจอ, ขนาดกระดานไม่เปลี่ยน (px เท่าเดิมเป๊ะ), margin ล่างยังเหลือ 62-181px, drag ด้วย pointer
  จริงยังตกช่องเป๊ะ

**VFX Framework + 3 เอฟเฟกต์ตัวแทน: Arcane Burst / Ground Slam / Arena Curse (เสร็จแล้ว)**
- **สถาปัตยกรรม**: เลเยอร์ภาพล้วนใน `src/game.js` (ต่อจาก FLOATING TEXT section) — VFX **อ่านผลจาก
  Combat ที่เกิดขึ้นแล้วเท่านั้น** ไม่คำนวณดาเมจ/สถานะเอง — ปิดได้ด้วย `VFX_ENABLED=false` (ทดสอบแล้ว:
  ปิดแล้วสกิลยังแคสต์/เวฟจบปกติ ไม่มี object โผล่เลย) + `CAMERA_SHAKE_ENABLED` แยกปิดเฉพาะสั่นกล้อง
- **API กลาง**: `spawnVFX(type, opts)` / `updateVFX(dt)` / `disposeVFX(fx)` / `clearAllVFX()` /
  `triggerScreenFlash(opts)` (เพดาน opacity 0.35 กันจอขาว, DOM overlay div เดียวสร้างครั้งเดียว) /
  `triggerCameraShake(opts)` (เพดาน 10px, decay quadratic, ใช้ `camera.setViewOffset` ระดับ px ล้วน
  ไม่ชนกับ position/frustum ที่ layoutBoard คุม — clearViewOffset เมื่อจบ)
- **Object pooling**: ทุกชิ้น (spark 48/glow 8/smoke 20/ring 10/circle 6/crack 4/rock 16/curseMark 6
  = 118 objects) สร้างครั้งเดียวตอนโหลดใน `vfxRoot` (Group แยก **ไม่เข้า tileMeshes/units จึงไม่โดน
  raycast**) — acquire = โชว์, release = ซ่อน, pool หมด = ข้ามชิ้นนั้น (จำกัด particle ต่อเอฟเฟกต์ในตัว)
  — texture ทั้งหมด procedural canvas (glow/spark/smoke/วงเวทมีรูน/รอยแตก 3 แบบ/ตราคำสาป) **ไม่มีไฟล์
  asset ใหม่ และไม่ต้องขอ texture จาก GPT รอบนี้เลย**
- **Combat hooks ที่ใช้ (4 จุด, ต่อท้ายโค้ดเดิมแบบ fire-and-forget)**: (1) cast start ใน `updateUnit`
  (จุดที่ตั้ง `action_state='casting'` — comment เดิมเขียนไว้เองว่า "stub visual until real skill VFX
  exists") → `VFX_SKILL_CAST[heroKey]`, (2) skill resolve หลัง `executeSkill()` → `VFX_SKILL_IMPACT[heroKey]`,
  (3) `executeBossSkill()` สาขา arena_curse หลัง apply debuff ครบ → ส่ง targets+duration จริงให้ VFX,
  (4) `animate()` เรียก `updateVFX(dt)` ด้วย dt ตัวเดียวกับ combat (คูณ speedMul แล้ว → จังหวะตรงทุก
  ความเร็ว, pause แล้วเอฟเฟกต์หยุดตาม) — `clearAllVFX()` ใน `onWaveCleared`+`onWaveFailed`
- **3 เอฟเฟกต์**: mage `arcane_cast` (วงเวทหมุน+แสงรวมตัว+ประกาย spiral เข้าหาตัว ยาวเท่า cast time
  จริง) + `arcane_burst` (วงแหวนซ้อน 3 ชั้นฟ้า/ม่วง + คลื่นขยาย + วงเวทใต้เป้า + ประกาย 16 เม็ด +
  flash ม่วง 0.22) — fighter `slam_cast` + `ground_slam` (วงกระแทกทอง + รอยแตก procedural สุ่ม 3 แบบ
  + หิน 9 ก้อนกระเด็นตกพื้นหาย + ฝุ่น + สั่นกล้อง 6px/0.3s + "hit stop เฉพาะภาพ": คลื่นค้างสเกลแรก
  ~60ms ลอจิกเกมเดินต่อปกติ) — บอส `arena_curse` (ย้อม background+fog ม่วงเข้มแบบ refcount รองรับ
  คำสาปซ้อน + วงคำสาปใหญ่หมุน + ควันดำลอยวน + ตราคำสาป (ตาในสามเหลี่ยม) ลอยตามหัวยูนิต — เช็ค
  `statuses` จริงทุกเฟรม: สถานะหลุด/ตายก่อนเวลา → ซ่อนทันที, จบสถานะ → สี/object คืนครบ ทดสอบยืนยัน
  กลางสนามรบว่า bg กลับ `1c1510` และ marks = 0)
- **บั๊กเก่าที่เจอระหว่างทำ (แก้แล้ว, display-only)**: hit-flash ของ basic attack/crit restore สีด้วย
  `color.set(0xffffff)` hardcode → มอนที่ไม่มีภาพ (Slime/OrcBrute ใช้ placeholderColor บน BoxGeometry)
  กลายเป็น**กล่องขาวถาวรหลังโดนตีครั้งแรก** และศพตอนล้มค้างเป็นสีขาว (เห็นชัดใน screenshot เทสต์ VFX)
  — แก้ 3 จุด (hit-flash/crit-flash/resetForWave) ให้ restore เป็น `placeholderColor || 0xffffff`
- **ผลทดสอบ (Playwright)**: เล่นจริง 3 เวฟ x4 มีสกิล 45 ครั้ง (arcane_cast 10/burst 10/slam_cast 13/
  ground_slam 12) ไม่มี NaN/error — draw calls 89 (นิ่ง) → 146 (พีคระหว่างเอฟเฟกต์ซ้อน) — pool คงที่
  118 objects ทุกเวฟ ไม่มี object ค้าง (activeVFX/visible = 0 หลังทุกเวฟ) — **geometry ที่โตขึ้นข้ามเวฟ
  (33→112) พิสูจน์แล้วว่าเป็นของเดิมก่อนมี VFX** (build เก่าโต 29→75 เหมือนกัน — น่าจะมาจาก unit
  spawn/remove ไม่ dispose geometry, อยู่นอกขอบเขตรอบนี้ ยังไม่แตะ) — ปิด VFX เล่นได้ปกติ 100% —
  regression ครบ: ซื้อ/วาง/ลากด้วย pointer จริง/Link/Monster Panel/ไม่มี horizontal scroll ที่จอกว้าง
  1600x700 / เกือบสี่เหลี่ยม 900x860 / จอเตี้ย 900x380

**Unit Resource Leak Fix + ยืนยัน layout กระดาน (เสร็จแล้ว — commit `fd0d4e9`)**
- **Root cause (พิสูจน์แล้ว ไม่ใช่เดา)**: `removeUnit()` แค่ `scene.remove(u.group)` โดยไม่ dispose
  อะไรเลย ขณะที่ `makeUnit()` สร้างของใหม่ต่อยูนิตทุกครั้ง — geometry 5-8 ชิ้น (body/เงา/หลอด HP คู่/
  หลอดมานาคู่/linkRing), material 6-10 ชิ้น, **texture clone ต่อยูนิต** (sprite sheet ฮีโร่ `frames>1`
  ใช้ `tex.clone()` เพื่อให้เฟรมอนิเมชันแยกกัน) และ star badge CanvasTexture ต่อยูนิต — จุดรั่วหนักสุด
  คือ `moveUnitTo()` (ลาก/วาง/ถอนทุกครั้ง = remove+recreate ทิ้งของทั้งชุด) และ `clearEnemies()` ทุกจบเวฟ
- **การจำแนก resource**: shared (ห้าม dispose ตอนลบยูนิตเดียว) = `SPRITES[*]` ต้นฉบับ (มอนสเตอร์
  1 เฟรมใช้ตรงๆ ร่วมกันหลายตัว), `EQUIP_BADGE_TEX`/`LINK_BADGE_TEX`, **BufferGeometry static ของ
  THREE.Sprite (r128 ใช้ตัวเดียวร่วมกันทุก sprite ทั้งเอนจิน — dispose ตัวเดียวพังหมด ต้องข้ามเสมอ)**,
  VFX pool ทั้งหมด / per-unit (ต้อง dispose) = geometry+material ทุกชิ้นใน group, sheet texture clone,
  star badge texture, floating text texture (ของ sprite ลูกใน group)
- **การแก้**: เพิ่ม `isSharedUnitTexture(t)` + `disposeObjectTree(root)` (traverse: dispose geometry
  เฉพาะ non-sprite, dispose material ทุกตัว, dispose map เฉพาะที่ไม่ shared) + `disposeUnitVisual(u)`
  (เรียก disposeObjectTree แล้วถอด floating text ของยูนิตนี้ออกจาก registry `floatingTexts`) — เรียก
  จาก `removeUnit()` **จุดเดียว** ครอบคลุมทุกเส้นทางลบ (ขาย/ย้าย-ลาก/จบเวฟชนะ-แพ้/summon despawn/
  Evolution/Star Combine) — New Run ใช้ `location.reload()` อยู่แล้วไม่ต้องแก้ — **ไม่แตะ VFX pool เลย**
- **ผลวัดจริง (Playwright, 10 เวฟ x4 ครบ boss เวฟ 5 + wave failed จริง + ปิด VFX เวฟ 6-7)**:
  churn 6 รอบ (ซื้อ→วาง→ลาก→ลาก→ถอน→ขาย) จบที่ **baseline เป๊ะ (33 geo/5 tex)** จากเดิมที่รั่วทุก
  รอบ — ระหว่างเกม geo 80-86 (ตามจำนวนยูนิตที่มีชีวิตจริง ไม่โตสะสมข้ามเวฟอีก) — ขายหมดเหลือ 50/18
  (ส่วนเกิน = VFX pool geometry + shared texture cache ที่อัปโหลดครั้งแรก ถูกต้องตามดีไซน์) — reload
  กลับ **33/5 เป๊ะ** — vfxPool คงที่ 118 ทุก checkpoint — ไม่มี NaN/error/object ค้าง/registry ค้าง —
  เกมเล่นต่อได้หลัง 10 เวฟ (shop เวฟ 10, losses 1 จาก fail ที่จงใจ) — hit-flash คืน `placeholderColor`
  ถูกต้อง (Slime ที่มีชีวิตสีเขียว `6fcf5a` หลังโดนตี — หมายเหตุ: ศพที่ตายกลาง flash ค้างสีส้มแดงจน
  จางหาย ~0.5s เป็นพฤติกรรมเดิมเล็กน้อย ไม่ใช่กล่องขาวถาวรแบบบั๊กเก่า)
- **Layout กระดาน: ตรวจสอบแล้ว layout ปัจจุบันผ่าน จึงไม่ปรับ board offset เพิ่ม** — วัดจริง 4 ขนาด:
  มือถือแนวนอน 844x390 gap = 6.0px / แนวตั้ง 390x844 gap = 365px / tablet 1194x834 gap = 6.0px /
  desktop 1280x800 gap = 6.0px (ตรงกับ `TOPBAR_CLEAR_PAD=6` ของระบบ adaptive เดิมพอดี — screenshot
  มือถือจริงจากผู้ใช้ก็ยืนยันภาพเดียวกัน) — resize กลางเกม (desktop→900x380) gap ยัง 6.0 — tile pick
  แม่นทั้งแถวบน (r=0)/กลาง (r=3)/ม้านั่ง (r=6) ทุกขนาดจอ — drag ด้วย pointer จริงตกช่องเป๊ะ — ไม่มี
  horizontal scroll — กระดานไม่ถูกย่อ (px เท่ารอบก่อนเป๊ะ)
- Regression ครบ: ซื้อ/วาง/ลาก/ขาย/Link/Monster Panel/สกิล+VFX ทั้ง 3 (ยิงจริงตลอด 10 เวฟ)/
  `VFX_ENABLED` on-off/`CAMERA_SHAKE_ENABLED` on-off (ปิดแล้ว trigger เป็น no-op จริง, `clearAllVFX`
  ล้าง shake/flash state ครบ)

**Death Hit-flash Cleanup: ศพไม่ค้างสี flash ระหว่าง fade-out (เสร็จแล้ว — commit `156e228`)**
- **Root cause (จากโค้ดจริง)**: hit-flash 2 จุด (โจมตีปกติ `0xff8866`/80ms และ crit `0xffe066`/100ms)
  restore สีผ่าน `setTimeout(() => target.alive && ...restore)` — ยูนิตที่**ตายภายในหน้าต่าง flash**
  ไม่ผ่านเงื่อนไข `alive` จึงไม่ถูกคืนสี ศพเลย fade ~0.5s ทั้งที่ยังติดสีส้มแดง (สังเกตพบตอนเทส VFX)
- **การแก้ (`src/game.js`)**: รวม logic เป็นฟังก์ชันกลาง `applyHitFlash(u, color, ms)` +
  `restoreBodyColor(u)` — เก็บ timer handle ไว้บนยูนิต (`u.hitFlashTimer`, flash ซ้อนกันจะ clear
  ตัวเก่าก่อนเสมอ ไม่มี timer หลุดมือ) แล้วผูกกับ lifecycle 3 จุด: (1) `handleUnitDeath()` เรียก
  `restoreBodyColor` ทันทีที่ตาย — คืนสีฐาน**ก่อน**เริ่ม death fade, (2) `disposeUnitVisual()`
  clearTimeout ค้าง — callback เก่าไม่มีทางไปแตะ material ที่ dispose แล้ว, (3) call site เดิม 2 จุด
  เปลี่ยนมาเรียก `applyHitFlash` แทนโค้ด inline — **ไม่ได้ถอดเงื่อนไข alive แบบดิบ และไม่แตะ duration
  ของ flash/fade เลย**
- ทดสอบผ่านครบ: ยูนิตมีชีวิตโดนตี flash แล้วคืนสีปกติ (timer เคลียร์), race จริง "flash แล้วตายใน
  เฟรมเดียวกัน" คืนสีฐานทันที (ฮีโร่ขาว/มอน placeholder เขียว) ศพสีถูกตลอด fade, ศัตรูตายจริงกลางสนาม
  ทุกศพสีเขียว `6fcf5a` ตลอด (เก็บตัวอย่างทุก 25ms ทั้งเวฟ), ตายพร้อมกัน 3 ตัวไม่มี error, team wipe
  ยัง trigger wave failed ปกติ (losses +1), เล่นต่อถึงเวฟ 7 ผ่าน boss เวฟ 5 ไม่มี NaN, churn
  ซื้อ→วาง→ลาก→ขาย ×6 memory เท่าเดิมเป๊ะ (39 geo/6 tex ก่อน-หลัง — leak fix จาก fd0d4e9 ไม่ถูก
  ทำลาย), VFX/CAMERA_SHAKE toggle ทำงานเหมือนเดิม, Link/Monster Panel ปกติ
